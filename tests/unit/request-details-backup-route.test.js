...
describe("GET /api/settings/request-details-backup", () => {
  ...
  it("returns a downloadable JSON backup created through the db backup API", async () => {
    mockBackup.mockImplementation(async (destinationPath) => {
      expect(destinationPath).toContain("request-details-backup-");
      await fs.writeFile(destinationPath, "json-bytes");
    ...
    expect(response.headers.get("content-disposition")).toContain("request-details-backup.json");
    expect(body).toBe("json-bytes");
