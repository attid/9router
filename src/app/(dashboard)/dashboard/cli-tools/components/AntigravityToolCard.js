...
export default function AntigravityToolCard({
  ...
  const loadSavedMappings = async () => {
    try {
      const res = await fetch(withBasePath("/api/cli-tools/antigravity-mitm/alias?tool=antigravity"));
      if (res.ok) {
  ...
  const fetchStatus = async () => {
    try {
      const res = await fetch(withBasePath("/api/cli-tools/antigravity-mitm"));
      if (res.ok) {
        const data = await res.json();
        setStatus(data);
      }
  ...
  const handleSaveMappings = async () => {
    ...
    try {
      const res = await fetch(withBasePath("/api/cli-tools/antigravity-mitm/alias"), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <Image
              src={withBasePath("/providers/antigravity.png")}
              alt={tool.name}
              width={32}
              height={32}
