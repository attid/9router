...
export default function DroidToolCard({
  ...
  const checkDroidStatus = async () => {
    ...
    try {
      const res = await fetch(withBasePath("/api/cli-tools/droid-settings"));
      const data = await res.json();
      setDroidStatus(data);
    } catch (error) {
  ...
  const handleResetSettings = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(withBasePath("/api/cli-tools/droid-settings"), { method: "DELETE" });
      const data = await res.json();
      if (res.ok) {
      ...
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <Image src={withBasePath("/providers/droid.png")} alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e) => { e.target.style.display = "none"; }} />
          </div>
