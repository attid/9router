...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setOidcRedirectUri(`${window.location.origin}${apiPath("/api/auth/oidc/callback")}`);
    }
  }, []);
  ...
  const testOidcConnection = async () => {
    ...
    try {
      const saveRes = await fetch(apiPath("/api/settings"), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const res = await fetch(apiPath("/api/auth/oidc/test"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
