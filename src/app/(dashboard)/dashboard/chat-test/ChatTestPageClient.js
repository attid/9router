...
export default function ChatTestPageClient() {
  ...
  // Track whether the persisted state has been read so the save effect
  // doesn't overwrite localStorage with the empty initial values on mount.
  const [stateLoaded, setStateLoaded] = useState(false);
  // Same race guard for the selected API key: the async loader runs after
  // the save effect, so without this the empty initial value would erase
  // the saved key id before the loader could read it back.
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        // Mark hydrated regardless: a fetch failure shouldn't strand the save
        // effect waiting for a state it'll never see.
        setKeyHydrated(true);
      }
    };
  ...
  // Persist on every change once the initial restore is done.
  useEffect(() => {
    if (!stateLoaded || typeof window === "undefined") return;
    try {
      window.localStorage.setItem(
        STATE_STORAGE,
        JSON.stringify({ apiMode, model, prompt })
      );
    } catch {
      // ignore quota / privacy-mode failures
    }
  }, [stateLoaded, apiMode, model, prompt]);
