...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig }) {
  ...
  const exchangeTokens = useCallback(async (code, state) => {
    if (!authData) return;
    try {
      const res = await fetch(withBasePath(`/api/oauth/${provider}/exchange`), {
        method: "POST",
  ...
  const completeXaiManualCode = useCallback(async (code) => {
    ...
    try {
      const res = await fetch(withBasePath("/api/oauth/xai/manual-code"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const startPolling = useCallback(async (deviceCode, codeVerifier, interval, extraData, deadlineMs) => {
    ...
    while (Date.now() < deadline) {
      ...
      try {
        const res = await fetch(withBasePath(`/api/oauth/${provider}/poll`), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      if (deviceCodeProviders.includes(provider)) {
        ...
        const deviceCodeUrl = new URL(withBasePath(`/api/oauth/${provider}/device-code`), window.location.origin);
      ...
      const authorizeUrl = new URL(withBasePath(`/api/oauth/${provider}/authorize`), window.location.origin);
      ...
      if (provider === "codex") {
        try {
          const proxyUrl = new URL(withBasePath(`/api/oauth/codex/start-proxy`), window.location.origin);
      ...
      if (provider === "xai") {
        try {
          const proxyUrl = new URL(withBasePath(`/api/oauth/xai/start-proxy`), window.location.origin);
          proxyUrl.searchParams.set("app_port", appPort);
          proxyUrl.searchParams.set("state", data.state);
          proxyUrl.searchParams.set("code_verifier", data.codeVerifier);
          proxyUrl.searchParams.set("redirect_uri", redirectUri);
  ...
  useEffect(() => {
    if (isOpen && provider) {
      ...
      if (provider === "codex") {
        fetch(withBasePath("/api/oauth/codex/stop-proxy")).catch(() => {});
      } else if (provider === "xai") {
        fetch(withBasePath("/api/oauth/xai/stop-proxy")).catch(() => {});
      }
    }
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      attempts += 1;
      try {
          const res = await fetch(withBasePath(`/api/oauth/${pollProvider}/poll-status?state=${encodeURIComponent(authData.state)}`));
        const data = await res.json();
        if (cancelled || callbackProcessedRef.current) return;
        if (data.status === "done") {
